...
use dcinside_model::{
    Comment, CommentKind, Document, DocumentKind, Gallery, GalleryKind, User, UserKind,
};
use serde::{
...
pub fn gallerykind_from_str<T: AsRef<str>>(s: T) -> GalleryKind {
    match s.as_ref() {
...
pub fn gallery_from_index(o: GalleryIndex) -> Gallery {
    Gallery {
        id: o.id,
        name: o.name,
        kind: o.kind,
...
pub fn userkind_from_id_and_ip<T1: AsRef<str>, T2: AsRef<str>>(
    id: &Option<T1>,
    ip: &Option<T2>,
) -> UserKind {
    match (id, ip) {
        (Some(t), None) if !t.as_ref().is_empty() => UserKind::Static,
        (Some(t), Some(u)) if !t.as_ref().is_empty() && u.as_ref().is_empty() => UserKind::Static,
        (None, Some(t)) if !t.as_ref().is_empty() => UserKind::Dynamic,
        (Some(t), Some(u)) if t.as_ref().is_empty() && !u.as_ref().is_empty() => UserKind::Dynamic,
        _ => UserKind::Unknown,
    }
}

pub fn commentkind_from_contents(contents: &str) -> CommentKind {
...
impl<'de> Deserialize<'de> for FromComment {
        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for CommentVisitor {
            type Value = FromComment;

            fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
                formatter.write_str("struct FromComment")
            }

            fn visit_map<V>(self, mut map: V) -> Result<FromComment, V::Error>
            where
                V: MapAccess<'de>,
            {
                ...
                while let Some(key) = map.next_key()? {
                    match key {
                        ...
                        "ip" => {
                            author_ip = Some(map.next_value()?);
                        }
                        "name" => {
                            author_nickname = Some(map.next_value()?);
                        }
                        "depth" => {
                        ...
                        "memo" => {
                            ...
                            kind = Some(commentkind_from_contents(&val));
                            contents = Some(val);
                        }
                        "reg_date" => {
                ...
                Ok(FromComment {
                    id: match id.ok_or_else(|| de::Error::missing_field("no"))? {
                        StringOrInt::String(s) => s
                            .parse()
                            .map_err(|_| de::Error::custom("field `no` should number_string"))?,
                        StringOrInt::Number(u) => u,
                    },
                    author: User {
                        kind: userkind_from_id_and_ip(&author_id, &author_ip),
        ...
        deserializer.deserialize_struct("FromComment", FIELDS, CommentVisitor)
    }
}
        ...
        .map(|node| -> Result<_, DocumentParseError> {
            ...
            let author = {
                ...
                User {
                    ...
                    kind: userkind_from_id_and_ip(&id, &ip),
                    id: id.and_then(|i| {
                        if i.is_empty() {
                            None
                        } else {
...
struct _CommentsResponse {
    comments: Option<Vec<FromComment>>,
    ...
    match body.pagination {
        ...
        Some(pagination) => {
            ...
            if let Some(mut comments) = body.comments {
                ...
                Ok((comments.into_iter().map(Comment::from).collect(), max_page))
            } else {
                Ok((Vec::new(), max_page))
            }
        }
...
mod tests {
    ...
    fn it_parses_problmetic_gallery1() {
        let res = parse_document_indexes(
            include_str!("../assets/gallery-problemtic.html"),
            "gallery_id",
        )
        .unwrap();
        let res: Vec<_> = res.into_iter().map(|d| d.unwrap()).collect();
